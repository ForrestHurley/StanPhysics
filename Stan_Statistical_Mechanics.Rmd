...
For reasons which can be well justified -- we will not justify them for the sake of simplicity and conciseness -- we approximate the density of states of the heat bath as $\log( \Omega_{bath}) = \alpha + \beta E_{bath}$. If the density of states of the heat bath is independent of the system of interest then $\Omega_{system}(E_{system})=\Omega_{bath}(E_{bath})$. Independence means that $\Omega_{system}\Omega_{bath}=\Omega_{total}$; there are no additional states possible due to interactions between the system and heat bath. Combining these various assumptions together, we find $p(E_{system}) \propto \Omega_A (E_{system}) \propto e^{-\beta E_{system}}$. $\beta$ is $\frac{1}{kT}$ where T is temperature and k is the Boltzmann constant. For our purposes, we will use units where $k=1$, so $\beta=\frac{1}{T}$. This choice of units means that temperature does not correspond directly to any familiar scale; fortunately, the units of temperature are not relevant for the vast majority of the analysis.

As an application, consider the canonical ensemble describing a one dimensional space with a single free particle. As previously mentioned $E = \frac{p^2}{2m}$. By substitution we get $P(p) \propto e ^ {-\beta \frac{p^2}{2m}}$. It is simply a normal distribution with mean 0 and variance $m/\beta=mT$. While the momentum of a free particle in an isolated system is obviously not described by a normal distribution, when energy can be randomly added or removed it will eventually approach normality.

We use Stan to sample from canonical ensembles using unnormalized probabilities at specific temperatures ($T=\frac1\beta$). Our uninformative prior is the uniform distribution. In a canonical ensemble the log probability of any state is equal within a constant to $-\beta E$ where $E$ is the energy of the state. We define the likelihood as $p(\beta | E) \sim e^{-\beta E}$; at a fixed temperature we sample states with various energies. The following code calculates this likelihood function.
...
One particularly simple statistical mechanical model is the Ising Model. It consists of an K-dimensional lattice of elements which either have spin up or spin down. These elements could be thought of as atoms in a metal; if they are generally aligned there is a net magnetic field. The total energy of the system is the sum of interactions between neighboring elements where $S_i,S_j\in\{-1,1\}$.
$$E=- J\sum_{<i,j>} S_i S_j$$
$J$ is a scalar describing the strength of the interactions and the sum is only over the neighboring spins. If the spins are in the same direction (have the same sign) then the energy is lower. Rather than defining rather arbitrary rules to describe the boundaries of the lattice, it is wrapped around such that no boundaries exist. This approach to handling the edges is called a periodic boundary condition. The figure below shows such the result for a 1 dimensional Ising Model. 

![\label{fig:is_cylinder}A single dimensional Ising Model with periodic boundary conditions. Two dimensional models are usually placed on a torus (donut).](Images/ising_cylinder.png){width=3.25in}
...
The XY Model is similar to the Ising Model, but with an extra degree of freedom for the spins. The energy of any given state is related to the dot products of the spins of nearest neighbors on the lattice. Two nodes on the lattice are nearest neighbors if and only if their coordinates differ by no more than 1 in a single dimension. The nodes (1,3) and (2,3) are nearest neighbors, while (1,3) and (2,4) are not.
$$E=-J\sum_{<i,j>}\hat{\sigma}_i\cdot\hat{\sigma}_j$$ 
Each spin is a unit vector in the XY plane rather than simply a binary up or down. If these vectors are further constrained to be on the X axis while maintaining unit length then it is fairly easy to see that the energy equation simplifies to the Ising Model.

![A 2D grid of spins in the 2D plane taken from a simulation of the XY Model](Images/xy_grid.png){width=3.25in}

The Ising Model has a second order phase transition with changing temperature. It also has a first order phase transition as an external magnetic field is applied. However, the XY Model on a 2 dimensional lattice has no first or second order phase transitions. Instead it has a  Kosterlitz-Thouless transition. The Kosterlizt-Thouless(KT) transition relates to the appearance of vortices, swirling patterns on the lattice [@Kosterlitz1973]. At the critical temperature $T_{KT}$ vortex pairs unbind and move apart. The number of measurable vortices increases drastically. Previous work has shown that for the XY Model on a 2D lattice, $T_{TK}\approx 0.893$ [@Jensen]. Our results presented later closely match this value. In the remainder of this document we work exclusively with the 2D XY Model. The reader is reminded that the dimensionality of the spin vectors is separate from the dimensionality of the lattice.
...
The KT transition arises from the unpairing of many vortices. Equivalently, paired clockwise and counter-clockwise vortices appear and move apart. We identify vortices by looking for lattice cells where the corner spin vectors tend to point clockwise or counterclockwise. Specifically we calculate the shortest difference in angles in the counterclockwise direction and sum. In the figure above, the angle between vectors A and B would be -45 degrees. This sum can either be $2\pi$, $0$, or $-2\pi$ (technically it can also be $4\pi$ and $-4\pi$ but those cases are exceedingly unlikely). On a lattice with periodic boundary conditions, there must be the same number of counter-clockwise vortices ($2\pi$) and clockwise vortices ($-2\pi$)[@Imriska2009]. As we are only examining periodic lattices, we ignore the clockwise vortices to simplify the code.
...
We define several helper functions for counting the number of counter-clockwise vortices on a spin lattice. \textit{get_coordinates} handles moving around lattice squares even at the periodic boundaries. \textit{find_angle} calculates the counter-clockwise angle between the vectors at coordinates \textit{a} and \textit{b}.
...
The data for the model holds the temperature and x and y dimensions of the lattice. The sampling parameters are the spins on a lattice of that size.
...
The specific heat is the amount of energy which must be added to the system to raise the temperature by 1 degree. To cast it in terms of variance, consider that the higher the specific heat, the more energy is released by a small change in temperature. It appears reasonable that the random fluctuations in energy would then be larger at any given temperature.
...
At a temperature of 2, which is above the critical temperature $T_{KT}$ of the KT transition, we found a vortex density of around 0.085. Despite the complexity of the sampling distribution, the resulting chain has a non-negligible effective sample size.
...
Increasing the amount of energy eventually leads to an almost linear increase in the number of unpaired vortices. Above energy densities of -1.5 per lattice point, 1 unit of energy density increase is equivalent to a vortex density increase of slightly over 0.1. Equivalently, adding a single vortex to the overal system requires slightly less than 10 units of energy.
